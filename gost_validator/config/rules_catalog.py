...
RULES_CATALOG_BY_VALIDATOR: dict[str, list[RuleResult]] = {
    ...
    "AppendicesValidator": [
        ...
        RuleResult("APPX-009", "ПРИЛОЖЕНИЯ", "После обозначения есть отдельная строка статуса в скобках", Severity.CRITICAL.value),
    ...
    "FigureValidator": [
        ...
        RuleResult("FIG-004", "РИСУНКИ", "Подпись рисунка с прописной буквы без точки в конце", Severity.CRITICAL.value),
        RuleResult("FIG-005", "РИСУНКИ", "Подпись рисунка соответствует паттерну: 1 / 1.1 / А.3(если в приложении)", Severity.CRITICAL.value),
    ],
    "TableValidator": [
        RuleResult("TABLE-001", "ТАБЛИЦЫ", "Подпись таблицы расположена сверху и выровнен влево", Severity.CRITICAL.value),
        RuleResult("TABLE-002", "ТАБЛИЦЫ", "Подпись соответсвует патерну  1 / 1.1 / А.3(если в приложении)", Severity.CRITICAL.value),
        RuleResult("TABLE-003", "ТАБЛИЦЫ", "Наименование таблицы(при его наличии) приводиться через тире", Severity.CRITICAL.value),
        RuleResult("TABLE-004", "ТАБЛИЦЫ", "Наименование таблицы с прописной буквы без точки в конце", Severity.CRITICAL.value),
        RuleResult("TABLE-005", "ТАБЛИЦЫ", "В таблице отсутствуют диагональные линии", Severity.CRITICAL.value),
        RuleResult("TABLE-006", "ТАБЛИЦЫ", "Заголовки граф начинаются с прописной буквы", Severity.CRITICAL.value),
